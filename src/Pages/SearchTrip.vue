<template>
  ...
  <MapComponent
    ...
    :points-of-interest="[
      {
        id: '1',
        position: { lat: 48.86, lng: 2.35 },
        iconUrl: 'https://i.pravatar.cc/100?img=5',
        popupText: 'Point 1',
      },
      {
        id: '2',
        position: { lat: 48.85, lng: 2.36 },
        iconUrl: 'https://i.pravatar.cc/100?img=8',
        popupText: 'Point 2',
      },
    ]"
  />
...
async function fetchPlaces(q: string): Promise<Suggestion[]> {
  ...
  return items.map((item) => {
    ...
    const street = [addr.house_number, addr.road || addr.pedestrian]
      ...
      .join(" ");
    const city =
      [addr.city, addr.town, addr.village, addr.county].filter(Boolean)[0] ||
      "";
...
function onFromSelect(selected: object) {
  const item = rawFromResults.value.find(
    (p) => p.display_name === selected.value
  );
  if (item) {
...
async function onSearch() {
  ...
  await getTrajetByProximite(form.start.lat, form.start.lng)
    .then((data) => {
